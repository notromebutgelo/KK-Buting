...
export default function Topbar({
  ...
  const handleBellClick = async () => {
    ...
    // Capture bell position before opening so the portal can align to it
    if (bellRef.current) {
  ...
  return (
    <>
      <header className="flex h-16 flex-shrink-0 items-center justify-between border-b border-[color:var(--kk-border)] bg-white/88 px-6 backdrop-blur">
        ...
        <div className="flex items-center gap-4">
          {/* Notification bell — button only; dropdown rendered via portal below */}
          <button
            ref={bellRef}
            type="button"
      ...
      {/* ── Portals — rendered at document.body so backdrop-blur stacking context can't clip them ── */}

      {mounted && notifOpen ? createPortal(
        <div
          ref={notifDropdownRef}
          style={{ position: 'fixed', top: notifPos.top, right: notifPos.right, zIndex: 9999 }}
          className="w-[340px] overflow-hidden rounded-[20px] border border-[color:var(--kk-border)] bg-white shadow-[0_20px_60px_rgba(1,67,132,0.22)]"
        >
          {/* Header */}
